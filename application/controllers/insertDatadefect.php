...
{
 	public function getMaxdefectactual($da_wi_no , $da_lot_no , $da_seq_no , $da_code){
	   	$sql = "select max(dt_id) as rs_id from defect_transactions where dt_wi_no ='{$da_wi_no}' and dt_lot_no = '{$da_lot_no}' and dt_seq_no = '{$da_seq_no}' and dt_code = '{$da_code}' and dt_status_flg =' 1'";
		$query = $this->TBK_FA01->query($sql);
		$get = $query->result_array();
		return $get["0"]["rs_id"];
	}
	 	public function getMindefectactual($da_wi_no , $da_lot_no , $da_seq_no , $da_code){
		$sql = "select min(dt_id) as rs_id from defect_transactions where dt_wi_no ='{$da_wi_no}' and dt_lot_no = '{$da_lot_no}' and dt_seq_no = '{$da_seq_no}' and dt_code = '{$da_code}' and dt_status_flg =' 1'";
		$query = $this->TBK_FA01->query($sql);
		$get = $query->result_array();
		return $get["0"]["rs_id"];
	}
	public function insertDefectactual(){
		...
		$da_st_dt = $this->getMindefectactual($da_wi_no , $da_lot_no , $da_seq_no  , $da_code);
		$da_en_dt = $this->getMaxdefectactual($da_wi_no , $da_lot_no , $da_seq_no  , $da_code);
		if (intval($da_qty) > 0){
			$da_status_flg = "1";
		}else{
			$da_status_flg = "5";
...
//		exit();
	...
	public function getReferencetagprintdefectminid($wi , $lot , $seq){
		$this->TBK_FA01 = $this->load->database('test_new_fa02', true);
		$sql = "Select min(da_id) as da_id from defect_actual where da_wi_no = '{$wi}' and da_lot_no = '{$lot}'  and da_seq_no = '{$seq}'";
		$query = $this->TBK_FA01->query($sql);
		$get = $query->result_array();
		return $get["0"]["da_id"];
	}
		public function getReferencetagprintdefectmaxid($wi , $lot , $seq){
		$this->TBK_FA01 = $this->load->database('test_new_fa02', true);
		$sql = "Select max(da_id) as da_id from defect_actual where da_wi_no = '{$wi}' and da_lot_no = '{$lot}'  and da_seq_no = '{$seq}'";
		$query = $this->TBK_FA01->query($sql);
		$get = $query->result_array();
		return $get["0"]["da_id"];
	}
	public function inserttagdefect(){
		...
		$dti_st_da = $this->getReferencetagprintdefectminid($dti_wi_no , $dti_lot_no ,$dti_seq_no );//"0"; //$_GET["dti_st_da"];
		$dti_en_da =  $this->getReferencetagprintdefectmaxid($dti_wi_no , $dti_lot_no ,$dti_seq_no );//"0"; //$_GET["dti_en_da"];
